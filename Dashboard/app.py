...
def buy_n_sell(
    ...
    df = df.sort_values("trade_date").sort_values(
    ...
    fig.add_trace(
        go.Scatter(
            x=df["trade_date"][df["Golden_Position"] == 1],
    ...
    # Layout setup
    fig.update_layout(
        title=f"Biểu đồ nến với tín hiệu mua/bán cho mã {stock_code}",
...
def lstm_prediction_plotly(df, train_ratio=0.75, epochs=1):
    ...
    if df.shape[0] < window_size:
        st.write(
            "Not enough data to train the model. Need at least window_size data points."
        )
        return
    ...
    x_validate = np.reshape(
        x_validate, (x_validate.shape[0], x_validate.shape[1], 1)
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=train.index, y=train["closing_price"], mode="lines", name="Train"
        )
    )
    fig.add_trace(
        go.Scatter(
            x=valid.index, y=valid["closing_price"], mode="lines", name="Valid"
        )
    )
    fig.add_trace(
        go.Scatter(
    ...
    fig.update_layout(
        title="Dự đoán giá cổ phiếu bằng LSTM",
        xaxis_title="Thời gian",
        yaxis_title="Giá",
...
def lstm_future_prediction(df, train_ratio=0.75, epochs=1, future_months=1):
    ...
    for _ in range(future_days):
        next_prediction = model.predict(
            last_window_data.reshape(1, window_size, 1)
        )
...
stock_code = st.sidebar.selectbox(
    "Mã cổ phiếu",
    options=data["stock_code"].unique(),
    index=data["stock_code"].unique().tolist().index("VCB"),
)
...
stock_data = (
    data[data["stock_code"] == stock_code]
...
lstm_future_prediction(
    stock_data, train_ratio=0.75, epochs=10, future_months=future_months
)
...
if st.button("Phân tích cổ phiếu"):
    with st.spinner("Đang phân tích..."):
        response = handle_analyst(formatted_system)
        st.session_state.analysis_result = (
            response  # Lưu kết quả vào session_state
        )
