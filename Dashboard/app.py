...
def get_request_token(stock_code):
...
token = get_request_token(stock_code)
...
# Dữ liệu bạn muốn gửi (dùng định dạng form-url-encoded)
body = {
    'code': stock_code,
    'interval': '1',
...
#xoá cột trading date
columns_to_drop = ['TradingDate', 'Timetype','Max','Min']

oneday_df.drop(columns=columns_to_drop, inplace=True)
##Chỉnh cột thời gian lấy giờ phút giây
...
#Chuyển cột Thời gian ra cột đầu tiên cạnh index
oneday_df = oneday_df[['Thời gian','Giá','KL Lô','KL tích luỹ']]

#In bảng dữ liệu ra streamlit
st.markdown(
    f"""
    <div style="
...
fig = make_subplots(
    ...
    subplot_titles=('Biểu đồ Giá theo Thời gian', 'Biểu đồ KL tích luỹ theo Thời gian'),
    row_heights=[0.6, 0.4]  # Điều chỉnh chiều cao cho từng biểu đồ
)
...
fig = make_subplots(
    rows=2, cols=1, 
    shared_xaxes=True,  # Chia sẻ trục X
    vertical_spacing=0.1,  # Khoảng cách giữa các subplots
    row_heights=[0.6, 0.4]  # Chiều cao mỗi subplot
)

# Vẽ biểu đồ đường cho 'Giá' (không tạo điểm chấm)
fig.add_trace(
    go.Scatter(x=grouped_df['Thời gian'], y=grouped_df['Giá'], 
               mode='lines', name='Giá', line=dict(color='blue')),  # Sử dụng 'lines' để vẽ đường không có điểm chấm
    row=1, col=1
)
...
fig.add_trace(
    go.Bar(x=grouped_df['Thời gian'], y=grouped_df['KL tích luỹ'],
           name='KL tích luỹ', marker=dict(color='orange')),
    row=2, col=1
)

# Cập nhật layout
fig.update_layout(
    title='Biểu đồ Giá và KL tích luỹ theo Thời gian',
    xaxis_title='Thời gian',
    yaxis_title='Giá',
    yaxis2_title='KL tích luỹ',
    height=800,  # Chiều cao tổng thể của cả figure
    showlegend=True
)
